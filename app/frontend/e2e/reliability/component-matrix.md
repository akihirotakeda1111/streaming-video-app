...
| Duplicate delivery / single owner | `app/frontend/e2e/reliability/duplicate-delivery.spec.ts` (fail-closed entry point) | `python app/scripts/run_reliability_e2e.py --scenario duplicate-delivery` | Required, not implemented: correlated busy/acknowledgement, ownership and attempts, one effective encode/publication, scoped cleanup | Unverified — observation and cleanup adapters required before live execution |
| Crash and long heartbeat | `recovery.spec.ts` (`@crash-recovery`, `@long-heartbeat`) | `python app/scripts/run_reliability_e2e.py --scenario crash-recovery` / `--scenario long-heartbeat` | worker identity, lease/visibility observations, bounded timeout evidence | Blocked — scoped control and observation adapters required |
