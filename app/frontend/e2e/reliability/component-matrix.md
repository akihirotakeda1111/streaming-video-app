...
| Crash and long heartbeat | `recovery.spec.ts` (`@crash-recovery`, `@long-heartbeat`) | `python app/scripts/run_reliability_e2e.py --scenario crash-recovery` / `python app/scripts/run_reliability_e2e.py --scenario long-heartbeat` | worker identity, lease/visibility observations, bounded timeout evidence | Blocked — scoped control and observation adapters required |
