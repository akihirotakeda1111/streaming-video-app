...
| Database update failure | `app/backend/worker/crates/worker/src/retry.rs::tests::persistence_panics_do_not_manufacture_a_terminal_or_retry_result` | `cargo test --manifest-path app/backend/worker/Cargo.toml persistence_panics_do_not_manufacture_a_terminal_or_retry_result` | no false terminal/retry result; no unauthorized publication | Covered |
